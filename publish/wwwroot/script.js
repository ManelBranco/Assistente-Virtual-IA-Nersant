...
function renderBotMessage(markdownText, metaText, modelName, apiUrl) {
    ...
    const apiMeta = apiUrl ? `<span class="meta-separator">•</span><span class="api-url">${apiUrl}</span>` : "";
    messageDiv.innerHTML = `
        ${botHtml}
        <div class="meta">${metaText}${modelMeta}${apiMeta}</div>
    `;
...
async function loadHistory() {
    ...
    if (!res.ok) {
        ...
        console.error("Erro a carregar histórico:", res.status, errorText);
        return;
    }
...
async function send() {
    ...
    if (!res.ok) {
        ...
        console.error("Erro no servidor /api/chat:", res.status, data.error || text);
    } else {
        data = await res.json();
    }
    ...
    const apiUrl = data.usedApiUrl || null;
    renderBotMessage(replyText, timeText, modelName, apiUrl);
...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    // Enviar mensagem com tecla Enter (Shift+Enter permite pular linha)
